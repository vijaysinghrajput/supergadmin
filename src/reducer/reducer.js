export const reducer = (state, action) => {
  if (action.type === "ADD_TO_CART") {
    if (!state.cartItems.find((o) => o.cartId === action.payload.cartId)) {
      ...
      state.totalAmmuont =
        parseInt(state.totalAmmuont) + parseInt(action.payload.price);
      return {
        ...state,
  ...
  if (action.type === "UPDATE_DATA") {
    ...
    const objIndex = state[action.data.type].findIndex(
    ...
    const removeByAttr = function (arr, attr, value) {
      ...
      while (i--) {
        if (
          ...
          state.totalAmmuont =
            parseInt(state.totalAmmuont) - parseInt(arr[i].totalPrice);
          state.totalItems =
            parseInt(state.totalItems) - parseInt(arr[i].itemQuant);
          arr.splice(i, 1);
        }
      }
    };
  ...
  if (action.type === "UPDATE_CART") {
    ...
    state.totalAmmuont =
      parseInt(state.totalAmmuont) - parseInt(obj.totalPrice);
    state.totalItems = parseInt(state.totalItems) - parseInt(obj.itemQuant);
    state.cartItems.splice(objIndex, 1, action.payload);
    state.totalAmmuont =
      parseInt(state.totalAmmuont) + parseInt(action.payload.totalPrice);
    state.totalItems =
      parseInt(state.totalItems) + parseInt(action.payload.itemQuant);
    return {
      ...state,
    };
  }
