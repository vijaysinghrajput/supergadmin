...
export const EditPlot = ({ editablePlot }) => {

    const { updateDataToCurrentGlobal, reloadData } = useContext(ContextData);
    ...
    const UpdatePlot = () => {
        console.log("help m", plotDetails);
        setIL(true);

        const formData = new FormData();

        plotDetails.update_project_feature_image && plotDetails.update_project_feature_image.map((item, i) => {
            formData.append(`update_project_feature_image[]`, item, item.name);
        })

        formData.append('project_feature_image', plotDetails.project_feature_image);
        formData.append('bussiness_id', plotDetails.bussiness_id);
        formData.append('id', plotDetails.id);
        formData.append('project_id', plotDetails.project_id);
        ...
        formData.append('plot_total_area', `${plotDetails.plot_total_area.split("/")[0]}/${plotDetails.plot_total_area_unit}`);
        formData.append('plot_sizes', plotDetails.plot_sizes);
        formData.append('plot_rate', `${plotDetails.plot_rate.split("/")[0]}/${plotDetails.plot_rate_unit}`);
        formData.append('road_size', plotDetails.road_size.split("/")[0] + "/ft");
        ...
        fetch(URL + "/APP-API/App/updatePlot", {
            method: 'POST',
            header: {
            ...
            .then((responseJson) => {
                ...
                if (responseJson.user) {
                    ...
                    console.log("added");
                    updateDataToCurrentGlobal({ type: "plots", payload: plotDetails }, { key: "id", value: plotDetails.id });
                    reloadData();
                }
                setIL(false);
                for (let i = 0; i < 10; i++) {
    ...
    useEffect(() => {
        setPlotDetails({
            ...editablePlot,
            plot_rate_unit: editablePlot.plot_rate_unit ? editablePlot.plot_rate_unit : editablePlot.plot_rate?.split("/")[1],
            plot_total_area_unit: editablePlot.plot_total_area_unit ? editablePlot.plot_total_area_unit : editablePlot.plot_total_area?.split("/")[1]
        });
            ...
            <div className="row">
                ...
                <div className="col-md-6">
                    <div className="mb-3">
                        ...
                        <input multiple type="file" onChange={e => setPlotDetails({ ...plotDetails, update_project_feature_image: [...e.target.files] })} className="form-control" id="address1ControlTextarea" />
                    </div>
                </div>{/*end col*/}
                <div className="col-md-6">
                    <div className="mb-3">
                ...
                <div className="col-md-6">
                    <div className="mb-3">
                        ...
                        <div className='d-flex'>
                            <input type="text" onChange={e => setPlotDetails({ ...plotDetails, plot_total_area: e.target.value })} value={plotDetails.plot_total_area?.split("/")[0]} className="form-control" placeholder="Project Area" id="address1ControlTextarea" />
                            <button type="button" class="btn btn-light dropdown-toggle"
                                data-bs-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">{plotDetails.plot_total_area_unit ? plotDetails.plot_total_area_unit : plotDetails.plot_total_area ? plotDetails.plot_total_area?.split("/")[1] : "Select Size"}</button>
                            <div class="dropdown-menu">
                ...
                <div className="col-md-6">
                    <div className="mb-3">
                        ...
                        <div className='d-flex'>
                            <input type="number" onChange={e => setPlotDetails({ ...plotDetails, plot_rate: e.target.value })} value={plotDetails.plot_rate?.split("/")[0]} className="form-control" placeholder="Rate" id="address1ControlTextarea" />
                            <button type="button" class="btn btn-light dropdown-toggle"
                                data-bs-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">{plotDetails.plot_rate_unit ? plotDetails.plot_rate_unit : plotDetails.plot_rate ? plotDetails.plot_rate?.split("/")[1] : "Select Size"}</button>
                            <div class="dropdown-menu">
                ...
                <div className="col-md-4">
                    <div className="mb-3">
                        ...
                        <input type="number" onChange={e => setPlotDetails({ ...plotDetails, road_size: e.target.value })} value={plotDetails.road_size?.split("/")[0]} className="form-control" placeholder="Road Size" id="address1ControlTextarea" />
                    </div>
                </div>{/*end col*/}
                <div className="col-md-4">
                    <div className="mb-3">
                ...
                <div className="col-lg-12">
                    <div className="text-center mt-2">
                        {isLoading ? <a href="javascript:void(0);" className="text-success"><i className="mdi mdi-loading mdi-spin fs-20 align-middle me-2" /> Updating </a> : <button type="button" onClick={UpdatePlot} className="btn btn-warning">Update</button>}
                    </div>
                </div>{/*end col*/}
            </div>{/*end row*/}
        </>
