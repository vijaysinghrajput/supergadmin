...
const CategoryManagement = () => {
  const { storeCategoryData, removeDataToCurrentGlobal, getToast } =
  ...
  const STORY_HEADERS = [
...

    {
      ...
      cell: (row) => {
        if (row.category_level == 0) {
          ...
          return <p className="text-success">{row.parent_name}</p>;
        }
      },
    },
    {
...

    {
      ...
      cell: (row) => {
        return <button className="btn btn-primary"> Products </button>;
      },
    },
    {
      prop: "dsd",
      ...
      cell: (row) => {
        if (row.status == 1) {
          ...
          <button className="btn btn-danger" onClick={ChangeStatus}>
            {" "}
            InActive{" "}
          </button>;
        }
