...
const ProductByParentCategory = () => {
    ...
    const changeStatusData = (value) => {
        ...
        if (value == 1) {
            const newParentData = storeProductsData.filter(obj => obj.status == 1 && obj.parent_category_id == subcatID)
            setShowData(newParentData);

        }
        else {
            const newChildData = storeProductsData.filter(obj => obj.status == 0 && obj.parent_category_id == subcatID)
